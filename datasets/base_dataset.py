...
class BaseDataset(Dataset):
    ...
    def __init__(self, root, crop_size, downsample, method, is_grey=False, unit_size=0, pre_resize=1, roi_map_path=None, gen_root=None):
        ...
        self.img_fns = glob(os.path.join(root, self.method, '*.jpg').replace("\\", "/")) + \
                          glob(os.path.join(root, self.method, '*.png').replace("\\", "/"))
        if self.gen_root is not None and self.method == 'train':
            self.img_fns += glob(os.path.join(self.gen_root, '*.jpg'.replace("\\", "/"))) + \
                              glob(os.path.join(self.gen_root, '*.png').replace("\\", "/"))
    ...
    def __getitem__(self, index):
        ...
        if img_fn.startswith(self.root):
            ...
            gt_fn = os.path.join(self.root, 'train', name[:-2] + '.npy').replace("\\", "/")
